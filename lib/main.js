...
exports.main = function(options, callbacks) {
    if (options.loadReason == "install"){
        //store original preference on install
...
exports.onUnload = function(reason) {
    if (reason == "disable") {
        eps.alert(null, "Search in new tab",
        ...
        //restore preference to what is was beforer installation
    }
...
function patch(browser_window) {
    ...
    if (searchbar) { //the search bar is in the window
        // console.log(searchbar.handleSearchCommand.toString());
        let ori = searchbar.handleSearchCommand.toString();
        let patched = ori.replace("function handleSearchCommand(aEvent)", "function (aEvent)");
        ...
        patched = "(" + patched + ")";
        searchbar.handleSearchCommand = browser_window.eval(patched); 
        //eval here should be fine.
        //the code must be evaled in the context of the browser window
        //since it depends on functions defined in chrome://browser/content/search/search.xml
        //replacing with evalInSandbox doesn't work.
    }
    if (browser_window.Omnibar !== undefined) { // patch Omnibar if it exist
        let ori = browser_window.Omnibar.handleSearchQuery.toString();
        let patched = ori.replace("openintab = currentBrowser.currentURI.spec != \"about:blank\";",
        ...
        browser_window.Omnibar.handleSearchQuery = browser_window.eval(patched); 
        //eval here should be fine. See reason from above
    }
